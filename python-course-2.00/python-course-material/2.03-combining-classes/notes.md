...
2. ```python
   ...
   ```
   (`library.books[0]` gives you the first `Book` object, then `.title`
   reaches into that object's attribute.)
3. It keeps the project organized — anyone reading it can find a class
   immediately just from its filename, instead of scrolling through one
   giant file. It also keeps each class focused on doing one thing well.
4. ```python
   from Dog import Dog
   ```
5. ```python
